...
# 识别编码：优先 UTF-8，回退 GBK（中文游戏常见）、Latin-1（兜底）
_ENCODING_CANDIDATES = ("utf-8-sig", "utf-8", "gbk", "latin-1")
...
class ScriptEditorAgent(BaseAgent):
    ...
    async def execute(self, task: AgentTask) -> AgentResult:
        ...
        if operation == "list":
            return self._do_list(game_dir, file_ref)
        if operation == "delete":
            return self._do_delete(game_dir, file_ref)
        if operation == "read":
            return self._do_read(game_dir, file_ref)
        if operation == "replace":
            return self._do_replace(game_dir, file_ref, data.get("source") or "")
        if operation == "create":
            return await self._do_create(game_dir, file_ref, instruction)
        # 默认 edit
        return await self._do_edit(game_dir, file_ref, instruction)

    # ------------------------------------------------------------------ #
    async def _do_edit(self, game_dir: str, file_ref: str, instruction: str) -> AgentResult:
        """改：读原文件 → LLM 生成新内容 → 备份 → 写入。"""
        ...
        try:
            result.backup_path = str(self._backup(target))
    ...
    async def _do_create(self, game_dir: str, file_ref: str, instruction: str) -> AgentResult:
        ...
        original = self._read_text(target) if target.exists() else ""
        try:
    ...
    def _do_delete(self, game_dir: str, file_ref: str) -> AgentResult:
        """删：备份后删除文件。"""
        target = self._resolve_file(game_dir, file_ref)
        if isinstance(target, str):
            return AgentResult(success=False, message=target, errors=[target])
    ...
    def _do_read(self, game_dir: str, file_ref: str) -> AgentResult:
        """查：读取文件内容原文。"""
        target = self._resolve_file(game_dir, file_ref)
        if isinstance(target, str):
            return AgentResult(success=False, message=target, errors=[target])
    ...
    def _do_replace(self, game_dir: str, file_ref: str, source_ref: str) -> AgentResult:
    ...
    def _do_list(self, game_dir: str, file_ref: str) -> AgentResult:
