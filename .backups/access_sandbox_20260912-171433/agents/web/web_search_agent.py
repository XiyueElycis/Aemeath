...
class WebSearchAgent(BaseAgent):
    ...
    async def execute(self, task: AgentTask) -> AgentResult:
        ...
        if url or operation == "download":
            return await self._download(url, game_dir, dest_name)
        # 否则视为搜索
    ...
    # ------------------------------------------------------------------ #
    async def _download(self, url: str, game_dir: str, dest_name: str) -> AgentResult:
        """下载 URL 文件到目标路径，覆盖前备份。"""
        if not url:
        ...
        try:
            if target.exists():
                backup = str(self._backup(target))
