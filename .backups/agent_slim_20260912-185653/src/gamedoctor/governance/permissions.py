...
执行层智能体之间**没有横向调用权**——安全/存档/Mod 等智能体的先后
...
MUTATING_AGENTS: frozenset[str] = frozenset({
    ...
    "dlc_manager",    # Mod/DLC 增删与版本同步
    "save_manager",   # 存档写入/迁移（备份也是写）
    "update_manager",  # 更新动作
    "script_editor",  # 文件编辑
})
...
SAFETY_PREREQUISITES: tuple[tuple[str, str, str], ...] = (
    ("dlc_manager", "security_agent", "装 Mod 前未做反作弊检测，有封号风险"),
    ("dlc_manager", "update_manager", "装 Mod 前未确认游戏版本，版本不匹配会崩溃"),
)
