...
class GovernanceCoordinator:
    ...
    async def run(
        ...
        *,
        ...
        thinking: List[str] = [f"[接旨] 任务票 {ticket.ticket_id} 建立，当前【{'编辑' if mode == 'edit' else '方案'}模式】"]
        ...
        while True:
            ...
            if draft.plan is None:
                ...
                return await self._single_agent_path(
                    ...
                    agents_desc=agents_desc, enable_search=enable_search,
    ...
    async def _single_agent_path(
        ...
        agents_desc: str, enable_search: bool, think_level: str, mode: str,
        ...
        ticket.transition(GovState.DOING, _INTAKE, f"智能体「{agent_name}」",
        ...
        ticket.transition(GovState.DONE, f"智能体「{agent_name}」", _INTAKE, "单任务回报")

        result_json = json.dumps(to_dict(result), ensure_ascii=False, default=str)
        try:
            reply = await asyncio.to_thread(
                generate_reply, client, message,
                self.manager.get_display_name(agent_name), result_json, model,
                game_dir, think_level, mode,
            )
        except LLMError as exc:
