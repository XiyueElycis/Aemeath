...
class TestReviewer:
    KNOWN = frozenset({
        "a", "b", "c", "dlc_manager", "security_agent", "update_manager",
        "installation", "save_manager", "script_editor",
    })
    ...
    def test_plan_mode_blocks_mutating_agents(self):
        plan = _plan([OrchestrationStage("s1", [_task("save_manager", 10, {})])])
    ...
    def test_dlc_without_security_is_warning_not_block(self):
        plan = _plan([OrchestrationStage(
            "s1", [_task("dlc_manager", 10, {}), _task("update_manager", 20, {})])])
        v = Reviewer().review(plan, mode="edit", known_agents=self.KNOWN)
...
class TestCoordinator:
    ...
    def test_template_rejected_when_required_agent_disabled(self):
        # 固定模板引用的 security_agent 不在白名单（被用户停用）：审议封驳即终止
        names = ["compatibility", "log_analyzer",
                 "network_expert", "performance_analyst", "community_agent"]
        coord, _ = _coord(names + ["security_agent"])  # 执行层已注册，但白名单不含
        client = FakeClient()
        out = _run(coord.run(
            "游戏报错闪退", client=client, model="m",
            agents_desc="", known_agents=set(names),
        ...
        assert out["ticket"]["state"] == "Cancelled"
        assert out.get("error") == "review_rejected"
        assert any("security_agent" in t for t in out["thinking"])
