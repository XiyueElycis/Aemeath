...
采用「确定性主干 + LLM 两端收口」：LLM 只出现在入口与出口——

1. **入口分流（正则先行）**：``detect_intent`` 命中工作流模板（安装/排障/
   调优/维护）时走确定性链路，选完之后的智能体顺序由
   ``OrchestrationStage.dependencies`` / ``task.requires`` 锁死，LLM 插不进来；
   未命中才降级问一次 LLM 做智能体级决策（单智能体路径）。
2. **执行**：工作流由编排器执行（阶段间串行、阶段内可并行、任务级门控
   降级）；单智能体由 LLM 决策结果直接路由。
3. **出口解释**：两种路径都复用 ``generate_reply``，把结构化结果转成
   自然语言回复。
...
class AgentChatService:
    ...
    async def chat(self, message: str, game_dir: str = "", history: Optional[List] = None,
        ...
        # 思考过程步骤列表：记录「分流 → 决策 → 执行 → 回复」全链路，供前端展开查看
        thinking: List[str] = []

        # 0. 入口分流（正则先行，免费且确定）：命中工作流模板就走确定性链路，
        #    LLM 只在出口解释结果；未命中才降级问一次 LLM 做智能体级决策。
        intent = detect_intent(message)
        if intent is not None:
            if mode == MODE_PLAN and intent in MUTATING_WORKFLOWS:
                thinking.append(f"[分流] 命中「{WORKFLOW_LABELS[intent]}」，但方案模式不执行含写操作的工作流，已拦截")
                return {
                    "reply": (
                        f"已识别到「{WORKFLOW_LABELS[intent]}」意图，但当前为【方案模式】，"
            ...
            return await self._run_workflow(intent, message, game_dir, client, model,
                                            think_level, mode, thinking)

        thinking.append("[分流] 正则未命中工作流模板，降级由大模型决策")
    ...
    # ------------------------------------------------------------------ #
    async def _run_workflow(self, intent: str, message: str, game_dir: str,
                            client: HttpLLMClient, model: str, think_level: str,
                            mode: str, thinking: List[str]) -> Dict[str, Any]:
        """执行确定性工作流：模板链路由代码锁死，LLM 仅在出口解释结果。

        这是「方案三」的主干路径：入口正则命中模板 → 编排器按
        ``OrchestrationStage`` / ``task.requires`` 确定性执行 →
        ``generate_reply`` 把结构化结果转成自然语言。全程至多 1 次 LLM 调用。
        """
        label = WORKFLOW_LABELS.get(intent, intent)
        game_name = Path(game_dir).name if game_dir else message[:24]
        thinking.append(f"[分流] 命中「{label}」，运行确定性工作流（智能体链路由代码固定，不由大模型选择）")

        plan = build_workflow(self.manager.orchestrator, intent, game_name,
                               error_message=message, game_dir=game_dir)
        context = GameContext(game_name=game_name)
        if game_dir:
            context.install_path = Path(game_dir)

        thinking.append(f"[执行] 工作流「{plan.description or plan.name}」开始，共 {len(plan.stages)} 个阶段")
        try:
            outcome = await self.manager.orchestrator.execute_workflow(plan, context)
        except Exception as exc:  # noqa: BLE001
            thinking.append(f"[执行] 工作流执行异常：{exc}")
            return {"reply": f"工作流执行出错：{exc}", "agent_used": f"workflow:{intent}",
                    "result": None, "thinking": thinking, "error": str(exc)}

        counts = outcome.get("counts", {})
        thinking.append(
            f"[执行] 工作流完成：{counts.get('succeeded', 0)} 成功 / "
            f"{counts.get('failed', 0)} 失败 / {counts.get('skipped', 0)} 跳过"
        )

        # 出口收口：把精简后的结构化结果交给 LLM 生成自然语言解释
        summary_json = self._workflow_summary_json(outcome)
        display = f"工作流「{label}」"
        try:
            reply = await asyncio.to_thread(
            ...
            reply = (
                f"工作流已执行完成（{counts.get('succeeded', 0)} 成功 / "
                f"{counts.get('failed', 0)} 失败 / {counts.get('skipped', 0)} 跳过），"
                f"但生成解释失败：{exc}"
            )
        thinking.append("[回复] " + (reply[:200] + "…" if len(reply) > 200 else reply))

        return {
            "reply": reply,
            "agent_used": f"workflow:{intent}",
            "result": {"plan": outcome.get("plan_name"),
    ...
    def _workflow_summary_json(outcome: Dict[str, Any]) -> str:
        """把工作流结果压成适合回喂 LLM 的精简 JSON（只留每个任务的成败与摘要）。

        完整 ``task_results`` 含各智能体的原始数据，直接回喂会撑爆上下文；
        出口解释只需要「哪个环节做了什么、成没成」。
        """
        stages: Dict[str, Any] = {}
