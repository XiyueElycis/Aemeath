...
from ..primitives.base import REGISTRY
...
class DefaultAuthorizationPolicy:
    ...
    def classify(self, action: RepairAction) -> ActionLevel:
        ...
        # 原语自身声明 L3（系统级、不可回滚动作）→ 不管方案里标成什么级别都拦截。
        # REGISTRY 可能尚未填充（如隔离单测），查不到时回退到动作自带级别。
        primitive = REGISTRY.get(action.primitive)
        if primitive is not None and primitive.default_level == ActionLevel.L3_FORBIDDEN:
            return ActionLevel.L3_FORBIDDEN
        return action.level
