...
3. requires 只能引用本计划内其他任务的 name；调查类（日志/兼容性/网络/性能安全体检/环境）应并行先行。
4. 安全顺序铁律：涉及 Mod 安装/更新时，perf_security（operation=security 反作弊检测）
   与 game_content（operation=version 定版本基线）必须排在 game_content（operation=dlc）之前；
   任何写操作之前必须先有 game_content（operation=saves）备份；
   安装类任务以 installation 开头、以安装验证收尾。
   注意：game_content 用 operation 区分子项 version / saves / dlc；
   perf_security 用 operation 区分 performance / security（不填则两项全检）。
    ...
    {{"name": "阶段英文slug", "description": "这一阶段做什么（中文）", "parallel": true,
      ...
      "tasks": [
        {{"name": "任务slug", "agent": "智能体注册名", "priority": 10,
          ...
          "operation": "可选：read/list/edit/create/download；game_content 用 version/saves/dlc，perf_security 用 performance/security",
...
_PLAN_MODE_CLAUSE = (
    "7. 当前为【方案模式】：只做只读调查。禁止规划 installation/game_content/script_editor "
    "等写操作智能体（game_content 的 version/dlc 只读子项也不在方案模式规划，统一在编辑模式处理），"
    "禁止 download；可使用日志分析、兼容性、perf_security、网络等只读智能体给出诊断方案。"
)
