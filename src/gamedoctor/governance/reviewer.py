...
class Reviewer:
    ...
    def review_outcome(self, outcome: Dict[str, Any]) -> ReviewVerdict:
        ...
        智能体通过 ``AgentResult.unverified`` 主动申报的「未实测占位结论」
        同样汇总为保留警告，防止占位能力被包装成检测结果交付。
        """
        v = ReviewVerdict()
        ...
        # task_results 值是 AgentResult 对象（非 dict）；skipped 任务没有真实结论，
        # 其未验证申报不重复计数。
        for name, result in (outcome or {}).get("task_results", {}).items():
            unverified = getattr(result, "unverified", None)
            if not unverified or getattr(result, "skipped", False):
                continue
            v.warnings.append(
                f"任务「{name}」含未实测项（{len(unverified)} 项），"
                f"不得作为诊断结论：{('、'.join(unverified))[:120]}"
            )
        return v
