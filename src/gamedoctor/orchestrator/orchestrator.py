...
class GameAgentOrchestrator:
    ...
    async def execute_workflow(self, plan: OrchestrationPlan, context: GameContext) -> Dict[str, Any]:
        ...
        for idx, stage in enumerate(plan.stages, start=1):
            ...
            # 阶段结束聚合回显：并行阶段的结果一次性汇成一条摘要，
            # 用户无需逐任务翻日志即可看到本阶段整体成败。
            tracker.log("phase",
                        f"阶段 {idx}/{total_stages}「{stage.name}」完成：{self._stage_brief(stage, results)}")
    ...
    @staticmethod
    def _stage_brief(stage: OrchestrationStage, results: Dict[str, AgentResult]) -> str:
        """阶段结果的一句话摘要：N 成功 / M 失败 / K 跳过。"""
    ...
    async def _execute_stage_sequential(self, stage: OrchestrationStage, context: GameContext,
        ...
        先按 ``priority`` 升序（同优先级按 name 稳定排序）再逐个执行，
        让工作流模板里的优先级字段真正决定串行顺序；每完成一个就并入
        ``task_results``，因此同阶段后续任务可以依赖前面任务的结果。
        """
        results: Dict[str, AgentResult] = {}
        local = dict(task_results)
        for task in sorted(stage.tasks, key=lambda t: (t.priority, t.name)):
    ...
    def create_diagnostic_workflow(self, game_name: str, error_message: str,
        ...
        return OrchestrationPlan(
            ...
            stages=[
                ...
                OrchestrationStage(
                    name="inspect",
                    description="按证据分头核查：性能安全体检 / 网络",
                    dependencies=["collect"],
                    tasks=[
                        # perf_security 一次巡检覆盖性能瓶颈 + 反作弊/恶意进程
                        _task("perf_security", 20, {**base, "op": "health"},
    ...
    def create_installation_workflow(self, game_name: str, error_message: str = "",
        ...
        顺序不可调换的关键约束（版本/存档/DLC 已收归 ``game_content`` 按 op 分流，
        性能与安全已收归 ``perf_security``；同一智能体的不同任务以 name 区分）：

        - ``perf_security``(security) 必须在 ``game_content``(dlc) 之前：反作弊兼容性
          未通过就装 Mod 有封号风险（PRD §4.7 高危拦截）。
        - ``game_content``(saves) 必须在任何写操作之前：PRD §4.7「任何写操作前必备份」。
        - ``game_content``(version) 必须在 dlc 任务之前：后者依赖前者定下的版本基线
          做 ``version_sync``。
            ...
            stages=[
                ...
                OrchestrationStage(
                    ...
                    tasks=[
                        ...
                        _task("game_content", 20, {**base, "op": "version"},
                              name="content_version", requires=["installation"]),
                        _task("perf_security", 30, {**base, "op": "security"},
                              name="security_check", requires=["content_version"]),
                        _task("game_content", 40, {**base, "op": "saves"},
                              name="content_saves", requires=["security_check"]),
                        _task("game_content", 50, {**base, "op": "dlc"},
                              name="content_dlc", requires=["content_saves"]),
                    ],
                ),
                ...
                OrchestrationStage(
                    ...
                    tasks=[
                        ...
                        _task("perf_security", 30, {**base, "op": "performance"},
                              name="perf_tune", requires=["launch_optimizer"]),
                    ],
                ),
        ...
        return OrchestrationPlan(
            ...
            stages=[
                OrchestrationStage(
                    ...
                    tasks=[
                        _task("compatibility", 10, base),
                        _task("perf_security", 10, {**base, "op": "performance"},
                              name="perf_baseline"),
                    ],
                ),
                OrchestrationStage(
        ...
        return OrchestrationPlan(
            ...
            stages=[
                OrchestrationStage(
                    ...
                    tasks=[
                        _task("compatibility", 10, base),
                        _task("game_content", 10, {**base, "op": "version"},
                ...
                OrchestrationStage(
                    ...
                    parallel=False,
                    tasks=[
                        _task("perf_security", 20, {**base, "op": "security"},
                              name="security_check", requires=["content_version"]),
                        _task("game_content", 30, {**base, "op": "saves"},
                              name="content_saves", requires=["security_check"]),
                        _task("game_content", 40, {**base, "op": "dlc"},
                              name="content_dlc", requires=["content_saves"]),
                    ],
                ),
                OrchestrationStage(
...
# 含写操作的工作流模板：diagnostic 全程只读，其余都会改文件 / 装 Mod /
# 下载内容。方案模式（plan）下这些模板必须被入口拦截，只放行 diagnostic。
MUTATING_WORKFLOWS: frozenset = frozenset({"installation", "optimization", "maintenance"})
