...
_INTENT_RULES: List[Tuple[Tuple[str, ...], str, str, str]] = [
    ...
    # 版本/存档/DLC 同属游戏内容管理，按任务名分流到 game_content 的不同 op
    (("dlc", "mod", "模组", "扩展包"), "game_content", "dlc_management", "DLC/Mod 管理"),
    (("启动", "加载慢", "进不去", "启动参数"), "launch_optimizer", "launch_optimization", "启动优化"),
    (("存档", "备份", "档丢了", "云存档"), "game_content", "save_management", "存档管理"),
    (("更新", "版本", "升级", "补丁"), "game_content", "update_management", "更新管理"),
    (("社区", "攻略", "教程", "视频", "讨论"), "community", "community_aggregation", "社区资源聚合"),
    (("好友", "社交", "语音", "联机好友"), "social", "social_management", "社交管理"),
    # 性能与安全同属运行体检，按任务名分流到 perf_security 的不同 op
    (("性能", "帧率", "fps", "卡顿", "掉帧", "优化画面"), "perf_security", "performance_analysis", "性能分析"),
    (("安全", "反作弊", "病毒", "恶意", "封号"), "perf_security", "security_check", "安全检查"),
...
| `/agents` | 列出所有已注册智能体 |
| `/run <能力标签>` | 直接按能力标签执行，如 `/run perf_security`、`/run game_content` |
| `/help` | 显示本帮助 |
| `/exit` 或 `/quit` | 退出会话 |
"""
...
class AgentREPL:
    ...
    def _handle_command(self, text: str) -> bool:
        ...
        if cmd == "/help":
            ...
            if not arg:
                self.console.print("[yellow]用法：/run <能力标签>，如 /run perf_security[/]")
            else:
