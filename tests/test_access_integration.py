...
def test_classify_kinds() -> None:
    ...
    # game_content 合并体按 op 细分：缺省（全做含备份）/saves 为写，version/dlc 只读
    assert classify("game_content", {}) == "write"
    assert classify("game_content", {"op": "saves"}) == "write"
    assert classify("game_content", {"operation": "save_management"}) == "write"
    assert classify("game_content", {"op": "version"}) == "read"
    assert classify("game_content", {"op": "dlc"}) == "read"
    # perf_security 巡检全程只读
    assert classify("perf_security", {"op": "security"}) == "read"
    assert classify("perf_security", {}) == "read"
